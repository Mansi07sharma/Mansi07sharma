<h1 align="center"> Hi there, I'm <a href="https://www.linkedin.com/in/mansi-sharma-03b5822b1/">Mansi Sharma</a> </h1>
<p align="center">
Software Developer • AI/ML Enthusiast • Full Stack Developer
</p>

<!--- Adding Header Elements -->
<p align="center">
  <a href="https://www.linkedin.com/in/mansi-sharma-03b5822b1/">LinkedIn</a> •
  <a href="https://github.com/Mansi07sharma">GitHub</a> •
  <a href="https://leetcode.com/u/mansi-sh07/">LeetCode</a> •
  <a href="mailto:mansi07sh@gmail.com">Email</a>
</p>

-----------------------------------------------------------
**About Me**<img src="https://raw.githubusercontent.com/sanjay-kv/sanjay-kv/main/Assets/illustration.png" min-width="300px" max-width="300px" width="350px" align="right"> <br>
Computer Science graduate focused on building scalable software systems, AI-powered applications, and practical machine learning solutions. Experienced in full-stack development, backend engineering, and intelligent automation with hands-on internship experience in AI and cloud technologies.

Currently interested in:
- Software Engineering
- Machine Learning & Generative AI
- Backend Systems
- Full Stack Development<br>

## Tech Stack

| Category | Technologies |
|---|---|
| Languages | Python, Java, C++, JavaScript, SQL |
| Frontend | React.js, Next.js, Tailwind CSS |
| Backend | Node.js, Express.js, REST APIs, JWT Authentication |
| Databases | MongoDB, MySQL |
| AI / ML | Scikit-learn, Pandas, NumPy, SHAP, IBM AutoAI, LangChain, LangGraph, AI Agents, OCR |
| Automation & Workflow | n8n |
| Tools & Platforms | Git, GitHub, Postman, IBM Cloud, Vercel, Linux |
## Experience

### 🏢 SourceSoft Solutions Pvt. Ltd. — Software Developer Intern  
**Sep 2025 – Nov 2025**

- Developed scalable AI-powered backend systems using Python and multi-agent workflow architecture.  
- Built an NLP-driven agent capable of converting natural language into optimized MongoDB queries for dynamic data retrieval.  
- Worked on AI-based UI generation and validation systems to create components directly from user prompts.  
- Integrated SendGrid APIs and webhook pipelines for automated real-time email notifications and workflow automation.  
- Collaborated in Agile development processes, contributing to backend optimization, debugging, testing, and feature improvements.  

### ☁️ Edunet Foundation (IBM SkillsBuild) — Cloud & AI Intern  
**Jul 2024 – Aug 2024**

- Processed and analyzed large-scale unstructured datasets using IBM Cloud Object Storage and cloud-based AI tools.  
- Built and evaluated machine learning models using IBM AutoAI for predictive analytics and intelligent decision-making tasks.  
- Developed an AI-powered travel assistant using Watsonx Assistant to provide personalized real-time travel recommendations.  
- Gained practical exposure to cloud computing, AI workflows, and deployment-oriented development using IBM technologies.

## Selected Projects

◆ **Financial Risk Scoring Engine** — ML-based credit risk prediction system with SHAP explainability and risk-based approval logic.
◆ **System Design Simulator** — Interactive distributed system simulator for latency, throughput, and architecture visualization.  
◆ **FlowMind** — AI-powered DSA visualizer generating flowcharts, code explanations, and complexity analysis.  
◆ **InternSecure** — Intelligent fake internship detection platform using domain validation and LLM-based authenticity checks.  
◆ **LivWell** — Smart housing and property management platform with chatbot, voice search, and map integration.  
◆ **Chess Game with AI Chatbot** — Browser-based chess game integrated with AI-assisted gameplay interaction.  
◆ **Travel Chatbot** — AI travel assistant developed using IBM Cloud and Watsonx Assistant technologies.  
◆ **ML Prediction Models** — Predictive ML systems for salary estimation, house price prediction, and admission forecasting.  

<!--- Adding Tech Stack open Section -->
## Highlights

<table>
<tr>
<td> • <b>DSA & Problem Solving</b><br>
Solved 740+ coding problems across LeetCode & GeeksforGeeks with focus on algorithms and optimization.</td>
</tr>

<tr>
<td> • <b>Level 3 Conqueror</b><br>
Completed Coding Ninjas' 21-Day Coding Challenge demonstrating coding consistency and advanced problem-solving skills.</td>
</tr>

<tr>
<td> • <b>Open Source</b><br>
Contributed to multiple projects during GirlScript Summer of Code (GSSoC) with accepted pull requests and collaborative development experience.</td>
</tr>

<tr>
<td> • <b>AI & Cloud Certifications</b><br>
Certified in Generative AI and cloud technologies with hands-on experience in AI-powered applications.</td>
</tr>
</table>

<details>	
 <summary><b>🛠 Tech Stack/ Certifications</b></summary><br>
  
## Certification Badges 💻
<div style='display:flex; align-items:center; gap: 10px;' align='center'>
<a href="https://www.cloudskillsboost.google/public_profiles/f26aa205-1649-4d21-8f5f-900eeef0decd/badges/8565852">
<img src="https://cdn.qwiklabs.com/sxIAsi5jEWKJ%2Bh1XFul00Hw1LHqPosK7YaMEn9hRZ6I%3D" width="115px" height="108px" />
<a href="https://www.cloudskillsboost.google/public_profiles/f26aa205-1649-4d21-8f5f-900eeef0decd/badges/8571916">
<img src="https://cdn.qwiklabs.com/Q4p8W8RjqG4NV4dpXY3ZhkGNnew12IqoiexKVvOtCQw%3D" width="115px" height="108px" />
<a href="https://www.cloudskillsboost.google/public_profiles/f26aa205-1649-4d21-8f5f-900eeef0decd/badges/8585598">
<img src="https://cdn.qwiklabs.com/teuTnmFi96VycJaZff9bmOfUQMz1tgPiTtsCH2iBBY8%3D" width="115px" height="108px" />
<a href="https://www.cloudskillsboost.google/public_profiles/f26aa205-1649-4d21-8f5f-900eeef0decd/badges/8643274">
<img src="https://cdn.qwiklabs.com/Zd4Z2gdc58uW%2Fu5AcTNOvKnjokXECt8jfHGoYqqgz6w%3D" width="115px" height="108px" />
<a href="https://www.cloudskillsboost.google/public_profiles/f26aa205-1649-4d21-8f5f-900eeef0decd/badges/8739748">
<img src="https://cdn.qwiklabs.com/Q%2FScwWC%2Fvpq%2BTQSLrzQ64SWm7j69wpa%2FVQL9ZZ8yYY0%3D" width="115px" height="108px" />
</div>
</details> 

## GSSOC(24) Badges 
<div style='display:flex; align-items:center; gap: 10px;' align='center'>
  <a href="https://api.badgr.io/public/assertions/Uprfs2wfT76NFwJ3SEjtUw?identity__email=yogesh11222211%40gmail.com">
  <img src="https://media.badgr.com/uploads/badges/assertion-Uprfs2wfT76NFwJ3SEjtUw.png?versionId=lDP_URrxmDeVp7sklw4LVuqg9g0kT6hG" width="100px" height="100px" /> 
  <img src="https://github.com/GSSoC24/Postman-Challenge/blob/7b99afdfc09cc774f5c8b2a639160d3dd6b25e24/docs/assets/1.png" width="100px" height="100px" />
  <img src="https://github.com/GSSoC24/Postman-Challenge/blob/84215853b038ecb844e97e80ae0d17e7ceaee0cc/docs/assets/2.png" width="100px" height="100px" />
  <img src="https://github.com/GSSoC24/Postman-Challenge/blob/7b99afdfc09cc774f5c8b2a639160d3dd6b25e24/docs/assets/3.png" width="100px" height="100px" />
  <img src="https://github.com/GSSoC24/Postman-Challenge/blob/7b99afdfc09cc774f5c8b2a639160d3dd6b25e24/docs/assets/4.png" width="100px" height="100px" />
  <img src="https://github.com/GSSoC24/Postman-Challenge/blob/7b99afdfc09cc774f5c8b2a639160d3dd6b25e24/docs/assets/5.png" width="100px" height="100px" />
</div>

<!--- Body End -->
<p align="center">
Passionate about building impactful technology, continuously learning, and evolving through real-world problem solving, scalable development, and AI-driven innovation.
</p>
